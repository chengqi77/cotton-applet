  ...
  methods: {
    initAnimation() {
      wx.createSelectorQuery().in(this).select('.i-noticebar-content-wrap').boundingClientRect((wrapRect) => {
        wx.createSelectorQuery().in(this).select('.i-noticebar-content').boundingClientRect((rect) => {
          const duration = rect.height / 20 * this.data.speed;
          const animation = wx.createAnimation({
            duration: duration,
            timingFunction: "linear",
          });
          this.setData({
            wrapWidth: wrapRect.height,
            height: rect.height,
            duration: duration,
            animation: animation
          }, () => {
    ...
    startAnimation() {
      ...
      if (this.data.animation.option.transition.duration !== 0) {
        ...
        const resetAnimation = this.data.animation.translateY(this.data.wrapWidth).step();
        this.setData({
          animationData: resetAnimation.export()
        });
      }
      this.data.animation.option.transition.duration = this.data.duration;
      const animationData = this.data.animation.translateY(-this.data.height).step();
      setTimeout(() => {
        this.setData({
